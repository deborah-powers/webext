...
function traiterDemandeAutorisationEnvironnementaleE5(){
	if (! document.body.innerText.includes ('Tableau du choix')){
		...
		setTimeout (function(){
			fillInputByLabel ('Rubrique', 'Prélèvement dans un cour');
		}, 500);
	/*	setTimeout (function(){
			const radioButtons = getRadioButtonsAndCheckboxes();
			radioButtons[28].clickOn();
			radioButtons[29].clickOn();
			for (var r=10; r< radioButtons.length; r++) log (r, radioButtons[r].labels[0].children[0].innerText);
		}, 500); */
	}, 500);
	}
	else{
